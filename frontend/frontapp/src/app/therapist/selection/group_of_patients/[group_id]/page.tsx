    ...
    useEffect(() => {
      const unwrapParams = async () => {
        try {
          ...
          console.log("resolvedParams:", resolvedParams); // Depuración
          setGrupoId(resolvedParams.group_id); // Usa la clave correcta para el ID
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        const response = await fetch(`${baseUrl}groups/${grupoId}`, {
          method: "PATCH", // Usa el método apropiado según tu API
          headers: {
        ...
        if (response.ok) {
          ...
          router.push('/therapist/selection/group_of_patients'); // Redirige a una página de listado de escenas
