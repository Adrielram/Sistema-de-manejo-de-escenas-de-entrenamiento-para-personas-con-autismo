...
export default function Page() {
  ...
  useEffect(() => {
    const fetchObjetivos = async () => {
      try {
        ...
        // Seleccionar el primer objetivo automáticamente
        if (mapped.length > 0) {
    ...
    fetchObjetivos();
  }, [currentPage, username, baseURL]);

  // Fetch de escenas cuando cambia el objetivo seleccionado
  useEffect(() => {
    const fetchEscenasPorObjetivo = async (page: number = 1) => {
      ...
      try {
        ...
        const mapped = data.map((esc: Escena) => ({
          id: esc.id,
          nombre: esc.nombre,
  ...
  }, [selectedObjetivo, currentPageEscenas, username, baseURL]);
  ...
  const handleEscenaClick = (id: number) => {
