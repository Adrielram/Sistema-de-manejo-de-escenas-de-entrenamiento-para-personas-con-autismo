...
export default function Page() {
  ...
  const verificarEscenaAsignada = async (escenaId: number) => {
    try {
      const response = await fetch(
        `${baseURL}verificar-escena/?user_id=${username}&escena_id=${escenaId}`
      );
      ...
      return { asignada: false, objetivo_id: null };
    }
  };
  ...
  const handleEscenaClick = async (id: number) => {
    ...
    if (escena && !escena.bloqueada) {
      ...
      const result = await verificarEscenaAsignada(id);
      if (result.asignada) {
        dispatch(setObjetivoId({ objetivoId: result.objetivo_id.toString() }));
      } else {
        // Si no está asignada, usar el objetivo padre
        dispatch(setObjetivoId({ objetivoId: selectedObjetivo?.id.toString() || "" }));
      }
    }
  };
